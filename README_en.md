## How to install?
Minimal installation (including wm, no tmpfs as root), go to [here](https://github.com/benatouba/flakes/tree/minimal)
...
```bash
nix-shell -p git
git clone  https://github.com/benatouba/flakes.git /mnt/etc/nixos/Flakes
...
rm -rf .git
```
8. Username modification: edit `/mnt/etc/nixos/Flakes/flake.nix` to modify **user** variable, hostname modification: edit `/mnt/etc/nixos/Flakes/hosts/laptop/hardware-configuration.nix` to modify* The **hostName** value in the **networking** property group, and set networking options.
